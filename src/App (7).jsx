      ...
      <div style={{ width:"100%", maxWidth:380 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div>
            ...
            <div style={{ width:"100%", padding:"14px", borderRadius:14, fontSize:28, textAlign:"center", letterSpacing:"0.4em", border:"1px solid rgba(255,255,255,0.15)", background:"rgba(255,255,255,0.07)", color:"#ede8f8", marginBottom:10, minHeight:56 }}>
              {newPin ? "●".repeat(newPin.length) : <span style={{color:"rgba(255,255,255,0.2)"}}>0000</span>}
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8 }}>
              {[1,2,3,4,5,6,7,8,9,"",0,"⌫"].map((n,i)=>(
                <button key={i} onClick={()=>{
                  if(n==="⌫") setNewPin(p=>p.slice(0,-1));
                  else if(n!=="") setNewPin(p=>p+String(n));
                }} style={{ padding:"16px", borderRadius:14, border:`1px solid ${C.border}`, background:n==="⌫"?C.redDim:C.bgCard, color:n==="⌫"?C.red:C.text, fontSize:20, fontWeight:700, cursor:n===""?"default":"pointer", opacity:n===""?0:1 }}>
                  {n}
                </button>
              ))}
            </div>
      ...
      <div style={{ width:"100%", maxWidth:380 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          <div>
            ...
            {/* 表示欄 */}
            <div style={{ width:"100%", padding:"14px", borderRadius:14, fontSize:20, textAlign:"center", letterSpacing:"0.15em", border:"1px solid rgba(255,255,255,0.15)", background:"rgba(255,255,255,0.07)", color:"#ede8f8", marginBottom:10, minHeight:52, wordBreak:"break-all" }}>
              {shopCode || <span style={{color:"rgba(255,255,255,0.2)"}}>例: MIRAI123</span>}
            </div>
            {/* 文字ボタン */}
            <div style={{ display:"grid", gridTemplateColumns:"repeat(6,1fr)", gap:5, marginBottom:6 }}>
              {"ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("").map(c=>(
                <button key={c} onClick={()=>setShopCode(p=>p+c)} style={{ padding:"10px 4px", borderRadius:10, border:`1px solid ${C.border}`, background:C.bgCard, color:C.text, fontSize:14, fontWeight:700, cursor:"pointer" }}>{c}</button>
              ))}
            </div>
            {/* 数字ボタン */}
            <div style={{ display:"grid", gridTemplateColumns:"repeat(6,1fr)", gap:5 }}>
              {"0123456789".split("").map(n=>(
                <button key={n} onClick={()=>setShopCode(p=>p+n)} style={{ padding:"10px 4px", borderRadius:10, border:`1px solid ${C.goldBorder}`, background:C.goldDim, color:C.gold, fontSize:14, fontWeight:700, cursor:"pointer" }}>{n}</button>
              ))}
              <button onClick={()=>setShopCode(p=>p.slice(0,-1))} style={{ padding:"10px 4px", borderRadius:10, border:`1px solid ${C.red}`, background:C.redDim, color:C.red, fontSize:14, fontWeight:700, cursor:"pointer", gridColumn:"span 2" }}>⌫ 削除</button>
            </div>
          </div>
          <div>
            ...
            {/* PIN表示 */}
            <div style={{ width:"100%", padding:"14px", borderRadius:14, fontSize:28, textAlign:"center", letterSpacing:"0.4em", border:"1px solid rgba(255,255,255,0.15)", background:"rgba(255,255,255,0.07)", color:"#ede8f8", marginBottom:10, minHeight:56 }}>
              {pin ? "●".repeat(pin.length) : <span style={{color:"rgba(255,255,255,0.2)"}}>0000</span>}
            </div>
            {/* テンキー */}
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8 }}>
              {[1,2,3,4,5,6,7,8,9,"",0,"⌫"].map((n,i)=>(
